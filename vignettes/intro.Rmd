...
The model __sleuth__ uses for performing differential analysis is a general linear model where there is error in the response. Formally, in the case of two conditions being assayed, for a single transcript in a single sample $i$ ($i \in \{1,2\}$), the (log) "true" abundance $y_i$ measured in read counts is modeled by
...
More generally, __sleuth__ can be used to analyze multiple conditions organized in complicated experimental designs. In the general case the model is

$$ Y = X \beta + \epsilon $$

where $Y, X, \beta$ and $\epsilon$ are all matrices with additional matrices $Q$ and $D$ describing the response error layered on top of $Y$, i.e. 

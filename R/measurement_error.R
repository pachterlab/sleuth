...
#' @export
bs_sigma_summary <- function(obj, transform = identity, norm_by_abundance = FALSE) {
  # if (norm_by_abundance) {
...
reads_per_base_transform <- function(reads_table, scale_factor_input,
  ...
  # if (norm_by_abundance) {
  #   reads_table <- dplyr::mutate(reads_table,
  #     # scaled_reads_per_base = (sum(est_counts) / (reads_per_base)) * reads_per_base)
  #     )
  # } else {
  #   reads_table <- dplyr::mutate(reads_table,
  #     scaled_reads_per_base = scale_factor * reads_per_base
...
get_scaling_factors <- function(obs_raw) {
  obs <- obs_raw
  obs <- dplyr::group_by(obs, sample)
  dplyr::summarize(obs, scale_factor = sum(est_counts) / sum(est_counts / eff_len))
}

gene_summary <- function(obj, which_column, transform = identity, norm_by_abundance = FALSE) {
  # stopifnot(is(obj, 'sleuth'))
  obj_mod <- obj
  if (norm_by_abundance) {
    tmp <- obj$obs_raw
    ...
    function(i) {
      ...
      k$bootstrap <- lapply(k$bootstrap, function(b) {
        ...
        reads_per_base_transform(b, scale_factor, which_column,
          obj$target_mapping, norm_by_abundance)
      })
